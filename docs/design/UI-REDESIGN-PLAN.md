...
> **2026-09-24 更新：**本文作为历史视觉与交互蓝图参考。当前概念和实施顺序以 [入口网关认知对齐与实施计划](ENDPOINT-GATEWAY-ALIGNMENT-PLAN.md) 为准：一份接入配置一张提供商复合卡片、独立对外入口卡片、从左到右可编辑调用 DAG、两类入口演练比较及可扩展语言列表。不能以旧页面草图替代实际入口配置，也不能以设计稿证明功能已完成。
